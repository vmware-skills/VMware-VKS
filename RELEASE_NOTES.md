...
audit row under that tool's name (16 commands, `@audited`). Commands that reach nothing remote say so, with a reason (1, `@cli_local`).
...
(`/plugin install vmware-vks@vmware-skills`). The skill and its MCP server arrive in
